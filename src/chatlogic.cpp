...
using UniqueGraphNodePtr = std::unique_ptr<GraphNode>;
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](UniqueGraphNodePtr& node) { return node->GetID() == id; });
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            auto parentNode = std::find_if(_nodes.begin(), _nodes.end(), [&parentToken](UniqueGraphNodePtr& node) { return node->GetID() == std::stoi(parentToken->second); });
                            auto childNode = std::find_if(_nodes.begin(), _nodes.end(), [&childToken](UniqueGraphNodePtr& node) { return node->GetID() == std::stoi(childToken->second); });
